    ...
    def propagate_agents(
        ...
        for agent_token, agent in self.agents.items():
            if agent.is_active(iteration) and agent.has_valid_path():
                ...
                # If ego is not in the same lane as agent then ignore ego
                for lane in agent.get_route():
                    if intersecting_agents.contains("ego") and \
                        not (lane.contains_point(agent.to_se2().point) and lane.contains_point(ego_state.center.point)):
                        intersecting_agents.remove(["ego"])                
