...
from nuplan.common.actor_state.state_representation import StateSE2, StateVector2D, TimePoint, ProgressStateSE2
from nuplan.common.actor_state.tracked_objects_types import TrackedObjectType
from nuplan.common.actor_state.oriented_box import OrientedBox
from nuplan.common.maps.abstract_map_objects import RoadBlockGraphEdgeMapObject, LaneGraphEdgeMapObject, PolylineMapObject
...
from nuplan.common.geometry.transform import transform, rotate_angle
...
from nuplan.planning.simulation.path.utils import trim_path, convert_se2_path_to_progress_path
...
class IDMMobilPlanner(AbstractIDMPlanner):
    ...
    def _initialize_lane_change(self, desired_ego_state: EgoState, current_ego_state: EgoState,
                                current_ego_idm_state: IDMAgentState, occupancy_map: OccupancyMap, unique_observations: UniqueObjects) -> None:
        """
        Initializes the ego path to perform a lane change 
        :param ego_state: The ego state at the start of the scenario.
        """
        route_plan, _ = self._breadth_first_search(desired_ego_state)
        ego_speed = current_ego_state.dynamic_car_state.rear_axle_velocity_2d.magnitude()
        speed_limit = route_plan[0].speed_limit_mps or self._policy.target_velocity
        self._policy.target_velocity = speed_limit if speed_limit > ego_speed else ego_speed
        discrete_path = []
        ...
        # -------------------------------------------------------------
        current_p_state = ProgressStateSE2(current_ego_state.center.x, current_ego_state.center.y, current_ego_state.center.heading, 0)
        ...
        desired_p_state.progress = start_manouver_p_state.distance_to(desired_ego_state.center) + start_manouver_p_state.progress
        # Add the points from this newly created path to the discrete path
        path_from_current_to_desired = InterpolatedPath([current_p_state, start_manouver_p_state, desired_p_state])
        ...
        intersecting_agents =  occupancy_map.intersects(
            self._get_expanded_ego_path_in_AL(current_ego_state, current_ego_idm_state, path_from_current_to_desired)) 
        if intersecting_agents.size > 0 and \
            any([unique_observations[agent_id].velocity.magnitude() < 1 for agent_id in intersecting_agents.get_all_ids() 
                 if not agent_id[:9] == "red_light"]):
            return False

        for progress in np.arange(0.5, desired_p_state.progress, 0.5):
            discrete_path.append(path_from_current_to_desired.get_state_at_progress(progress))
        
        # Cut initial edge to start from ego_vehicle position
        initial_edge = convert_se2_path_to_progress_path(route_plan[0].baseline_path.discrete_path)
        initial_edge_linestring = path_to_linestring(initial_edge)
        progress_along_initial_edge = initial_edge_linestring.project(Point(*desired_ego_state.center.point.array))
        initial_edge = [StateSE2(state.x,state.y,state.heading) for state in initial_edge if state.progress >= progress_along_initial_edge]
    ...
    def _get_starting_edge(self, ego_state: EgoState) -> LaneGraphEdgeMapObject:
        ...
        for edge in [lane for roadblock in self._route_roadblocks for lane in roadblock.interior_edges]:
    ...
    def _get_planned_trajectory(
        ...
        if self.objective_lane.id not in [lane.id for lane in current_lane.parent.interior_edges]:
        ...
        if self.false_vehicle:
            ...
            unique_observations["false_vehicle"] = Agent(TrackedObjectType.VEHICLE, false_vehicle_OB, false_vehicle_velocity,
        ...
        if isinstance(current_lane, NuPlanLane) and len(current_lane.parent.interior_edges) > 1:
            assert adjacent_lanes[0] or adjacent_lanes[1], "Did not find starting edge"

        for right_bool, adjacent_lane in enumerate(adjacent_lanes): # Right bool: 0-> Left / 1-> Right

            if not adjacent_lane: continue
            
            # Condicitons to consider lane change
            position_in_baseline = self.objective_lane.baseline_path.get_nearest_pose_from_position(ego_state.center)
            is_lane_change_happening = position_in_baseline.distance_to(ego_state.center) > 0.5 or \
                                        principal_value(abs(position_in_baseline.heading - ego_state.center.heading)) > 0.1
            navigation_benefit_of_changing_lane[right_bool] = (self._distances_to_lane_on_route[current_lane.parent.id][current_lane.index-1] - \
                                                               self._distances_to_lane_on_route[current_lane.parent.id][adjacent_lane.index-1])
            does_adjacent_lane_leads_to_route = not navigation_benefit_of_changing_lane[right_bool] < 0 

            if isinstance(current_lane, NuPlanLane) and not is_lane_change_happening and does_adjacent_lane_leads_to_route:
            
                if "incentive_criterion" not in locals():
                    # Initializations -------------------------------------------------------------
                    incentive_criterion = [-1000, -1000] # [Left lane change, Right lane change]
                    # Calculate ac
                    leading_agent = self._get_leading_object(ego_idm_state, ego_state, occupancy_map, unique_observations)
                    new_idm_state = self._policy.solve_forward_euler_idm_policy(IDMAgentState(0, ego_idm_state.velocity), leading_agent,
                                                                                self._planned_trajectory_sample_interval)
                    ac = (new_idm_state.velocity - ego_idm_state.velocity) / self._planned_trajectory_sample_interval
                    ac_tilde = [None, None]
                    an = [None, None]
                    an_tilde = [None, None]
                    ego_state_in_adjacent = [None, None] # Ego state if ego would be in adjacent lane with the same progress
                    ...
                    follower_idm_state, follower_state = self._get_follower_agent(ego_state, occupancy_map, unique_observations)
                    ...
                    if follower_state:
                        ...
                        new_follower_idm_state = self._policy.solve_forward_euler_idm_policy(IDMAgentState(0, follower_state.velocity.magnitude()),
                                                                                leading_agent, self._planned_trajectory_sample_interval)

                        ao_tilde = (new_follower_idm_state.velocity - follower_idm_state.velocity) / self._planned_trajectory_sample_interval

                        ao = (follower_idm_state.velocity - follower_state.previous_state.velocity.magnitude()) / \
                            (ego_state.time_us - follower_state.previous_state.time_point.time_s)
                ...
                ego_state_in_adjacent[right_bool]: StateSE2 = \
                    adjacent_lane.baseline_path.get_nearest_pose_from_position(projected_ego_state.center)
                ego_progress_in_adjacent = \
                    adjacent_lane.baseline_path.linestring.project(Point(*ego_state_in_adjacent[right_bool].point.array))
                ego_idm_state_in_adjacent[right_bool] = IDMAgentState(ego_progress_in_adjacent, ego_state.dynamic_car_state.center_velocity_2d.x)

                ego_state_in_adjacent[right_bool] = EgoState.build_from_center(
                                            center=ego_state_in_adjacent[right_bool],
                                            center_velocity_2d=StateVector2D(ego_idm_state_in_adjacent[right_bool].velocity, 0),
                                            center_acceleration_2d=ego_state.dynamic_car_state.center_acceleration_2d,
                                            tire_steering_angle=0.0,
                                            time_point=current_time_point,
                                            vehicle_parameters=vehicle_parameters,)
                leading_adjacent_agent = self._get_leading_object_in_AL(ego_idm_state_in_adjacent[right_bool], 
                                                                            ego_state_in_adjacent[right_bool], occupancy_map,
                                                                            unique_observations, adjacent_lane)
                ...
                if leading_adjacent_agent.progress  == 0:
                    ...
                    continue
                elif leading_adjacent_agent.progress < 1: # Calculate new IDM step if relative distance < 1 m
                    new_idm_state = IDMAgentState(ego_idm_state.progress, ego_idm_state.velocity +                \
                                                    self._planned_trajectory_sample_interval * -self._policy._decel_max)
                else: # Normal IDM calculation
                    new_idm_state = self._policy.solve_forward_euler_idm_policy(IDMAgentState(0, ego_idm_state.velocity),
                                                                                leading_adjacent_agent, 
                                                                                self._planned_trajectory_sample_interval)
                    
                ac_tilde[right_bool] = (new_idm_state.velocity - ego_idm_state.velocity) / self._planned_trajectory_sample_interval
                ...
                follower_idm_state, follower_state = self._get_follower_agent(
                                                        ego_state_in_adjacent[right_bool], occupancy_map, unique_observations)
                if follower_idm_state:
                    # State of follower in left lane if ego vehicle change lanes
                    new_follower_IDM_state = \
                    self._policy.solve_forward_euler_idm_policy(IDMAgentState(0, follower_idm_state.velocity),
                                                                IDMLeadAgentState(follower_idm_state.progress, ego_idm_state.velocity,0), 
                                                                self._planned_trajectory_sample_interval)
                    
                    an_tilde[right_bool] = (new_follower_IDM_state.velocity - follower_idm_state.velocity) / self._planned_trajectory_sample_interval

                    # Acceleration of follower vehicle in adjacent lane without lane change
                    an[right_bool] = (follower_idm_state.velocity - follower_state.previous_state.velocity.magnitude()) / \
                            (ego_state.time_us - follower_state.previous_state.time_point.time_s)
                            # Using ego state to retrieve time information since when the observations get updated, the timestamp info
                            # of the metadata doesn't get updated
                else:
                ...
                incentive_criterion[right_bool] += self.politeness_factor*(an_tilde[right_bool] - an[right_bool] + ao_tilde - ao)
            elif does_adjacent_lane_leads_to_route:
                self.false_vehicle = False

        if "incentive_criterion" in locals():
            max_incentive_criterion = max(enumerate(incentive_criterion), key = lambda x: x[1])
            if max_incentive_criterion[1] >= self.changing_threshold:
                self.objective_lane = adjacent_lanes[max_incentive_criterion[0]]
                desired_ego_state_in_adjacent = self._get_ego_state_after_lane_change(ego_state_in_adjacent[max_incentive_criterion[0]], 
                                                                                        ego_idm_state_in_adjacent[max_incentive_criterion[0]],
                                                                                        adjacent_lanes[max_incentive_criterion[0]])
                success = self._initialize_lane_change(desired_ego_state_in_adjacent, ego_state, ego_idm_state, occupancy_map, unique_observations)
                if success:
                    ego_idm_state.progress = 0
                    planned_trajectory = [ego_state]
                    self.false_vehicle = False
            elif navigation_benefit_of_changing_lane[max_incentive_criterion[0]] > 0 and not is_lane_change_happening and self.push_lane_change: 
    ...
    def _get_ego_state_after_lane_change(self, ego_state_in_adjacent: EgoState, ego_idm_state_in_adjacent: IDMAgentState, adjacent_lane: NuPlanLane):
        """
        TODO
        """
        distance_to_change_lane = max(ego_idm_state_in_adjacent.velocity*1, 1) # TODO add to config
        # Calculate desired ego state after lane change
        adjacent_lane_path = InterpolatedPath(convert_se2_path_to_progress_path(adjacent_lane.baseline_path.discrete_path))
        ...
        desired_progress_state = adjacent_lane_path.get_state_at_progress(desired_progress)
        desired_ego_state_in_adjacent = EgoState.build_from_center(
                                                    center=StateSE2(desired_progress_state.x, desired_progress_state.y,
    ...
    def _get_ego_path_in_adjacent_lane(self, lane: LaneGraphEdgeMapObject, ego_idm_state: IDMAgentState) -> InterpolatedPath:
        """
        ...
        route_plan, _ = graph_search.search(self._route_roadblocks[-1], len(self._route_roadblocks[offset:]))
        ...
        return InterpolatedPath(convert_se2_path_to_progress_path(discrete_adjacent_ego_path))

    def _get_expanded_ego_path_in_AL(self, ego_state: EgoState, ego_idm_state: IDMAgentState,
                                                adjacent_ego_path: InterpolatedPath) -> Polygon:
        """
        ...
        path_to_go = trim_path(
            adjacent_ego_path,
            max(adjacent_ego_path.get_start_progress(), min(ego_idm_state.progress, adjacent_ego_path.get_end_progress())),
            max(
    ...
    def _get_leading_object_in_AL(
        ...
        expanded_ego_path_in_adjacent_lane = self._get_expanded_ego_path_in_AL(ego_state, ego_idm_state, 
                                                                                          adjacent_ego_path)
        intersecting_agents = occupancy_map.intersects(expanded_ego_path_in_adjacent_lane)
        ...
        if intersecting_agents.size > 0:
            ...
            # No leading agent
            return self._get_free_road_leading_idm_state_in_adjacent(ego_state, ego_idm_state, adjacent_ego_path)
        
    def _get_free_road_leading_idm_state_in_adjacent(self, ego_state: EgoState, ego_idm_state: IDMAgentState,
                                                     adjacent_ego_path: InterpolatedPath) -> IDMLeadAgentState:
        """
        Returns a lead IDM agent state when there is no leading agent.
        :return: A IDM lead agents state.
        """
        projected_velocity = 0.0
        relative_distance = adjacent_ego_path.get_end_progress() - ego_idm_state.progress
    ...
    def _get_follower_agent(
        ...
        if intersecting_agents.size > 0: # TODO this case is was not tested yet
            ...
            return [self._get_follower_idm_agent(ego_state, unique_observations[nearest_id], relative_distance), 
    ...
    def _get_follower_idm_agent(self, ego_state: EgoState, agent: SceneObject, relative_distance: float) -> IDMLeadAgentState:
        """
    ...
    def _initialize_route_plan(self, route_roadblock_ids: List[str]) -> None:
        ...
        for block in self._route_roadblocks:
            if block:
                ...
                lanes_on_route = []
                if not isinstance(block , NuPlanRoadBlockConnector): block.interior_edges.sort(key=lambda x: x.index) 
                for edge in block.interior_edges:
                    ...
                    for out in edge.outgoing_edges:
                        ...
                        if (block.id == self._route_roadblocks[-1].id and not self._goal) or \
                            (block.id == self._route_roadblocks[-1].id and edge.contains_point(self._goal)):
                            lanes_on_route.append(1)
                        else: lanes_on_route.append(0)

                if isinstance(block , NuPlanRoadBlockConnector): continue
                ...
                if not all(v == 0 for v in lanes_on_route):
                    ...
                    for idx, edge in enumerate(block.interior_edges): # TODO not entering here if it is full of ones 
                        if idx < min_IOLOR:
        ...
        assert not [edge for edge in self._route_roadblocks[-1].outgoing_edges if edge in self._route_roadblocks]
        ...
        for roadblock in reversed(self._route_roadblocks[:-1]):
            ...
            if roadblock.id in self._distances_to_lane_on_route and all(v==0 for v in self._distances_to_lane_on_route[roadblock.id]):
                distances_to_route = self._distances_to_lane_on_route[roadblock.id]
                for lane in roadblock.interior_edges:
                    distances_to_route[lane.index-1] = \
                    min([self._distances_to_lane_on_route[out.outgoing_edges[0].get_roadblock_id()][out.outgoing_edges[0].index-1] \
                         if out.outgoing_edges[0].get_roadblock_id() in self._distances_to_lane_on_route else 0 for out in lane.outgoing_edges])
    ...
    def _annotate_occupancy_map(
        ...
        for data in traffic_light_data:
            if (
                data.status == TrafficLightStatusType.RED and 
                (self._map_api.get_map_object(str(data.lane_connector_id), SemanticMapLayer.LANE_CONNECTOR).parent
                in self._route_roadblocks or 
                self._map_api.get_map_object(str(data.lane_connector_id),SemanticMapLayer.LANE_CONNECTOR).incoming_edges[0].parent 
        ...
        for id in occupancy_map.get_all_ids():
            if id.split("_")[0] == "red":
                t_light_lane_c = self._map_api.get_map_object(id.split("_")[2], SemanticMapLayer.LANE_CONNECTOR)
                paralel_lanes_to_t_light = t_light_lane_c.incoming_edges[0].outgoing_edges
                
                if t_light_lane_c.parent not in self._route_roadblocks and \
                len([True for edge in paralel_lanes_to_t_light if "red_light_"+edge.id in occupancy_map.get_all_ids()]) < \
