...
class ModificationsSerializableDictionary():
    ...
    list_of_modifications = {"a":"amount_of_agents", "d":"density", "g":"goal", "o":"observation", "s":"special_scenario"}
    ...
    def add_scenario_specifics(self, string):
        ...
        for idx, letter in enumerate(string):
            if idx % 2 != 0: continue # TODO this doesn't work if amount of agents has more than one digit
            if letter == "a": 
    ...
    def to_string(self) -> str:
        ...
        for letter, name in self.list_of_modifications.items():
            if name in self.dictionary: 
                if name == "special_scenario": return f"s{self.dictionary['special_scenario']}"
                else: string+=f"{letter + str(self.dictionary[name])}"
        return string
