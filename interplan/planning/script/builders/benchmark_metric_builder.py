...
def build_metrics_engines(cfg: DictConfig, scenarios: List[AbstractScenario]) -> Dict[str, MetricsEngine]:
    ...
    for scenario in scenarios:
        ...
        # Add scenario metrics to high/low level metrics 
        OmegaConf.set_struct(metrics_in_scope, False)
        OmegaConf.set_struct(high_level_metric_in_scope, False)
        for key, value in scenario_metrics.items():
            if value.get("required_metrics", None):
                high_level_metric_in_scope.update({key:value})
            else:
                metrics_in_scope.update({key:value})
        OmegaConf.set_struct(metrics_in_scope, True)
        OmegaConf.set_struct(high_level_metric_in_scope, True)
